...
class SensorNivelComida(Sensor):
    def __init__(self, id, valor_min=0, valor_max=100):
        super().__init__(id, valor_min, valor_max, unidad="%")
        self.capacidad_maxima_gramos = 2000  # 2kg de comida máximo
        self.nivel_critico = 20  # 20% es nivel crítico
        self.nivel_minimo = 10   # 10% es nivel mínimo
    
    def leer_valor(self):
        """Leer nivel de comida desde el sensor ultrasónico"""
        try:
            # Intentar leer del puerto serial (Arduino/ESP32)
            ...
            if response:
                ...
                # Validar y guardar lectura
                if self.validar_rango(nivel_porcentaje):
                    self.guardar_lectura(nivel_porcentaje)
                    return nivel_porcentaje
            
            # Si no hay respuesta del hardware, simular datos (para desarrollo)
            return self._simular_lectura()
            
        except Exception as e:
            print(f"Error leyendo sensor de nivel de comida {self.id}: {e}")
            return self._simular_lectura()
    
    def _simular_lectura(self):
        """Simular lectura para desarrollo/testing"""
        # Simular un nivel que va disminuyendo gradualmente
        if hasattr(self, '_ultimo_nivel_simulado'):
            # Decrementar aleatoriamente entre 0.1% y 1%
            decremento = random.uniform(0.1, 1.0)
            self._ultimo_nivel_simulado = max(0, self._ultimo_nivel_simulado - decremento)
        else:
            self._ultimo_nivel_simulado = random.uniform(50, 100)
        
        nivel = round(self._ultimo_nivel_simulado, 1)
        self.guardar_lectura(nivel)
        return nivel
    
    def calcular_gramos_restantes(self):
        """Calcular gramos de comida restantes"""
        if self.valor_actual is not None:
            return round((self.valor_actual / 100) * self.capacidad_maxima_gramos, 1)
        return 0
    ...
    def actualizar_estado(self, valor):
        """Actualizar estado específico para nivel de comida"""
        if valor is None:
    ...
    def obtener_info_completa(self):
        ...
        return {
            ...
            'gramos_restantes': self.calcular_gramos_restantes(),
            'estado': self.estado,
            'necesita_rellenado': self.necesita_rellenado(),
            'esta_vacio': self.esta_vacio(),
            'capacidad_maxima': self.capacidad_maxima_gramos,
            'ultima_lectura': self.ultima_lectura.isoformat() if self.ultima_lectura else None
        }
    
    def __str__(self):
        gramos = self.calcular_gramos_restantes()
        return f"Comedero {self.id}: {self.valor_actual}% ({gramos}g) - Estado: {self.estado}"
