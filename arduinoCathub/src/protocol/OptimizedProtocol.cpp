...
CommandProcessor::CommandProcessor(SensorManager* sensors, LitterboxStepperMotor* litter, 
  FeederStepperMotor* feeder, WaterDispenserPump* water) 
    : sensorManager(sensors), litterboxMotor(litter), feederMotor(feeder), waterPump(water),
      initialized(false), feederEnabled(true), waterDispenserEnabled(true), litterboxState(1) {
}
...
void CommandProcessor::processCommand(String command) {
    ...
    // ===== FORMATO NUEVO: DeviceID:comando =====
    if (command.startsWith("LTR1:") || command.startsWith("FDR1:") || command.startsWith("WTR1:")) {
...
void CommandProcessor::processDeviceIDCommand(String command) {
    ...
    // ===== BEBEDERO (WTR1) =====
    else if (deviceID == "WTR1") {
        if (action == "all") {
            sendWaterDispenserStatus();
        }
        else if (action == "0") {
            disableWaterDispenser();
        }
        else if (action == "1") {
            enableWaterDispenser();
        }
        else {
            Serial.println("{\"device_id\":\"WTR1\",\"error\":\"INVALID_ACTION\",\"action\":\"" + action + "\",\"valid\":[\"all\",\"0\",\"1\"]}");
        }
    }
    
    // ===== DEVICE ID DESCONOCIDO =====
    else {
        Serial.println("{\"error\":\"UNKNOWN_DEVICE_ID\",\"device_id\":\"" + deviceID + "\",\"valid\":[\"LTR1\",\"FDR1\",\"WTR1\"]}");
    }
}
...
void CommandProcessor::startNormalCleaning() {
    ...
    litterboxState = 21; // Estado 2.1 (usamos 21 para representar 2.1)
...
void CommandProcessor::startDeepCleaning() {
    ...
    litterboxState = 22; // Estado 2.2 (usamos 22 para representar 2.2)
...
void CommandProcessor::enableFeeder() {
    feederEnabled = true;
    feederMotor->enable();
    Serial.println("{\"device_id\":\"FDR1\",\"action\":\"enable\",\"success\":true,\"enabled\":true}");
}
...
// ===== IMPLEMENTACIÓN BEBEDERO (WTR1) =====
void CommandProcessor::sendWaterDispenserStatus() {
    String response = "{";
    response += "\"device_id\":\"WTR1\",";
    response += "\"type\":\"waterdispenser\",";
    response += "\"enabled\":" + String(waterDispenserEnabled ? "true" : "false") + ",";
    response += "\"status\":{";
    ...
    response += "\"safe_to_pump\":" + String(isWaterSafeToOperate() ? "true" : "false");
    response += "},";
    response += "\"timestamp\":" + String(millis());
    response += "}";
    Serial.println(response);
...
void CommandProcessor::enableWaterDispenser() {
    waterDispenserEnabled = true;
    Serial.println("{\"device_id\":\"WTR1\",\"action\":\"enable\",\"success\":true,\"enabled\":true}");
}

void CommandProcessor::disableWaterDispenser() {
    waterDispenserEnabled = false;
    if (waterPump->isPumpRunning()) {
        waterPump->turnOff();
    }
    Serial.println("{\"device_id\":\"WTR1\",\"action\":\"disable\",\"success\":true,\"enabled\":false}");
}

// ===== VALIDACIONES DE SEGURIDAD =====
bool CommandProcessor::isCatPresent() {
    float distance = sensorManager->getLitterboxDistance();
    return (distance > 0 && distance <= 6.0); // Gato presente si distancia <= 6cm
}
...
bool CommandProcessor::isLitterboxSafeToClean() {
    ...
    // Condiciones seguras: humedad 15-80%, temperatura 10-35°C, gas < 800ppm
...
bool CommandProcessor::isWaterSafeToOperate() {
    return !sensorManager->isCatDrinking(); // Seguro si el gato no está bebiendo
}
...
void CommandProcessor::sendAllDevicesStatus() {
    ...
    response += "\"actuators\":{";
    response += "\"pump_running\":" + String(waterPump->isPumpRunning() ? "true" : "false") + ",";
    response += "\"can_pump\":" + String(isWaterSafeToOperate() ? "true" : "false");
    response += "}";
    response += "}";
...
void CommandProcessor::update() {
    ...
    if (now - lastUpdate >= 2000) {
        ...
        if (feederEnabled && isFeederSafeToOperate()) {
            ...
            // Si peso < 150g Y hay comida en el compartimento
            if (weight < 150.0 && foodDistance < 8.0) {
                feederMotor->feedPortion(1); // Dispensar una porción
                Serial.println("{\"auto_action\":\"FEEDER_REFILL\",\"weight\":" + String(weight) + "}");
            }
        }
        
        // ===== AUTO-LLENADO DEL BEBEDERO =====
        if (waterDispenserEnabled && isWaterSafeToOperate()) {
            String waterLevel = sensorManager->getWaterLevel();
            
            // Si el agua está seca, encender bomba por 3 segundos
            if (waterLevel == "DRY" && !waterPump->isPumpRunning()) {
                waterPump->turnOn(3000); // 3 segundos
                Serial.println("{\"auto_action\":\"WATER_REFILL\",\"level\":\"DRY\"}");
            }
        }
        
        // ===== MONITOREO DE SEGURIDAD DEL ARENERO =====
        if (litterboxState == 1 && isCatPresent()) { // Si está READY pero hay gato
